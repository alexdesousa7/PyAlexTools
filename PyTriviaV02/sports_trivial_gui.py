...
class SportsTrivialGUI:

    # Categoría "Sports" en Open Trivia DB proviene del juego original
    SPORTS_CATEGORY = 21  
    ...
    def start(self):
        ...
        self.questions = self.client.get_questions(amount=self.n, category=self.SPORTS_CATEGORY)
        ...
        self.window.title("Sports Trivial")
    ...
    def show_question(self):
        ...
        Label(self.window, text=f"Pregunta {self.index + 1}/{self.n}",
    ...
    def check_answer(self, selected):
        ...
        if self.index < self.n:
            ...
            messagebox.showinfo("Resultado final", f"Has acertado {self.score}/{self.n}")
            self.window.destroy()


if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()  # Para oculta la ventana principal
    game = SportsTrivialGUI(root)
    game.start()
    root.mainloop()
